	...
	*/
    $.widget( "bn.bnQSExpression" , {
		...
		*/
		_createExpressions: function(app, appName){
			exprChache= []; exprCount = 0; genericStr= ''; selector= '';
	...
	$.widget( "bn.bnQSHtmlSelect" , {
        ...
        _create: function () {
			...
			this.selected = ' selected="selected"', aFieldName = '';
			...
			if($.isArray(vars.app)){
				...
				for(i=0;i<vars.app.length;i++){
					...
					, "qInitialDataFetch": [{ qTop : vars.qTop, qLeft : vars.qLeft, qHeight : vars.qHeight, qWidth : vars.qWidth }]
						...
						selectIt = '', state ='', counter = 0, scounter = 0, sname='', emptyValue='';
						...
						strEmpty = '<option class="" value="empty">'+ emptyValue + fCounter +'</option>';
						vars.multiple == true ? this.strEmpty = '':false;
				...
				, "qInitialDataFetch": [{ qTop : vars.qTop, qLeft : vars.qLeft, qHeight : vars.qHeight, qWidth : vars.qWidth }]
					...
					vars.str = ''; selectIt = '', state ='', counter = 0, scounter = 0, sname='', emptyValue='';
					$.each(reply.qListObject.qDataPages[0].qMatrix, function(key, value) {
						value[0].qState == 'S' && state != 'S' ? state = 'S' : false;
						value[0].qState == 'L' && state != 'L' ? state = 'L' : false;
						if(value[0].qState == 'S'){ scounter ++; sname = value[0].qText; }
        ...
        _create: function () {
			...
			if($.isArray(vars.app)){
				...
				, "qInitialDataFetch": [{ qTop : vars.qTop, qLeft : vars.qLeft, qHeight : vars.qHeight, qWidth : vars.qWidth }]
					...
					genericStr = '', selectIt = '', state ='', counter = 0, scounter = 0, sname='';
					$('#'+ vars.uniqueId).removeClass('statusS statusX');
					vars.str = '';  expressions=''; counter = 0; sname=''; emptyValue=''; frequencies='';
					$.each(reply.qListObject.qDataPages[0].qMatrix, function(key, value) {
						counter > 0 ? genericStr +=',' :false;
						if(vars.showFrequencies == true){
							expression = JSON.stringify('=count( {$ <['+vars.appField+']={'+'\"'+ value[0].qText + '\"' + '}>' + '} ['+ vars.appField+'])');
						...
						if(vars.ownExpression == true){
							...
							expression = JSON.stringify(vars.useExpression.replace('+/bnp+/','['+ vars.appField+']={'+'\"'+ value[0].qText + '\"' + '}'));
							genericStr +=  '"'+vars.uniqueId+'-expression'+ counter  +'": ' + '{ "qStringExpression":' + expression + '}';
							expressions = '&nbsp;<span class="bnListbox-expr" id="'+vars.uniqueId+'-expression'+ counter  +'"></span>';
						}
						vars.str += '<li class="status'+ value[0].qState +'" data-value="'+value[0].qText+'"><span class="bn-listbox-label">' + value[0].qText + '</span> '+ frequencies+' '+ expressions +'</li>';
						counter++; 
					});
					...
					if(vars.showFrequencies == true || vars.ownExpression == true || vars.showCountInLabel == true || vars.showTotalInLabel == true){
						...
						if(vars.showCountInLabel == true){
							if(vars.showFrequencies == true || vars.ownExpression == true) 
								genericStr += ','; 
							expression = JSON.stringify('=count(['+ vars.appField+'])');
						...
						if(vars.showTotalInLabel == true){
							if(vars.showFrequencies == true || vars.ownExpression == true || vars.showCountInLabel == true) 
								genericStr += ','; 
							expression = JSON.stringify('=count( {1} ['+ vars.appField+'])');
	...
	$.widget( "bn.bnResourceBrowser" , {
        ...
        _create: function () {
			...
			html =  '<div id="'+ this.options.ids.wrapper + '"><div id="'+ this.options.ids.header + '"><div id="'
	...
	$.widget( "bn.bnQSToolBox" , {
        ...
        _create: function () {
			...
			bnBox = '<div id="bnQSToolBox" class="bnQSToolBox">';
